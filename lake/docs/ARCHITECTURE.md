...
6. **Report** — `usecase/report` drives outline + per-section generation (LLM + `ports.GraphTools`), writes Python-compatible artifacts via `ports.ReportRepository` (`adapters/reportstore`: `meta.json`, `outline.json`, `progress.json`, `section_NN.md`, `full_report.md`, `agent_log.jsonl`, `console_log.txt`). `adapters/neo4j/graphtools.go` implements keyword search + stats over `GraphStore.GetGraphData`. HTTP: `POST /api/report/generate`, `GET|POST /api/report/generate/status` (GET uses `?report_id=` for the fishtank poller), `GET /api/report/check/{simulationId}`, `GET /api/report/by-simulation/{simulationId}`, `GET /api/report/{id}`, `GET .../agent-log`, `GET .../console-log`, `POST /api/report/chat`. Not yet ported: list/download/delete/progress/sections/tools routes from Flask.
