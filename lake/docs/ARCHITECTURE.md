...
5. **httpapi** — Depends on `ports` (or use case structs that embed ports). Never import concrete adapters. HTTP is served with [Fiber](https://github.com/gofiber/fiber) (routing, body size limit, CORS).
...
5. **Simulation** — `usecase/simulation` orchestrates `ports.SimulationRepository` (`adapters/simulationfs`), `EntityReader` (Neo4j), `ProfileBuilder` + `SimulationConfigBuilder` (`adapters/simulationprep`, LLM), and `ports.SimulationRuntime` (`adapters/simrunner`: Python `run_parallel_simulation.py`, `run_state.json`, jsonl tail, filesystem IPC). HTTP: `/api/simulation/*` (create/prepare/start/stop, profiles/config, run-status, actions, posts, interview batch, env-status). Neo4j entity reads: `GET /api/graph/entities/{graphId}/...` and Flask-compatible `GET /api/simulation/entities/{graphId}/...` (the `/entities` route group is registered before `/:simulationId` paths). If `LAKE_BACKEND_ROOT` / scripts are missing, runtime falls back to `simrunner.Disabled` (prepare still works; start returns an error).
6. **Report** — `usecase/report` drives outline + per-section generation (LLM + `ports.GraphTools`), writes Python-compatible artifacts via `ports.ReportRepository` (`adapters/reportstore`: `meta.json`, `outline.json`, `progress.json`, `section_NN.md`, `full_report.md`, `agent_log.jsonl`, `console_log.txt`). `adapters/neo4j/graphtools.go` implements keyword search + stats over `GraphStore.GetGraphData`. HTTP is Flask-equivalent for fishtank paths; **Lake-only:** `GET /api/report/check` and `GET /api/report/by-simulation` use `?simulation_id=` so they do not collide with `/api/report/{reportId}/...`. Also: list, download, DELETE, progress, sections, single section, log streams, chat, `tools/search`, `tools/statistics`.

## Module entry

- `cmd/lake/main.go` — Load config, build `app`, run Fiber (`Listen` + graceful `Shutdown` on SIGINT/SIGTERM).

## Environment (LLM)

- Chat completions use `LLM_BASE_URL` after normalization in `internal/config` (`NormalizeOpenAIv1BaseURL`): trailing slashes stripped; if the path does not already end with `/v1`, it is appended so clients can POST to `{base}/chat/completions`.
- Put real inference hosts and keys only in `.env` (ignored by git), not in committed files.
