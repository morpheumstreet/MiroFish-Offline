...
5. **Simulation** — `usecase/simulation` orchestrates `ports.SimulationRepository` (`adapters/simulationfs`), `EntityReader` (Neo4j), `ProfileBuilder` + `SimulationConfigBuilder` (`adapters/simulationprep`, LLM), and `ports.SimulationRuntime` (`adapters/simrunner`: Python `run_parallel_simulation.py`, `run_state.json`, jsonl tail, filesystem IPC). HTTP: `/api/simulation/*` (create/prepare/start/stop, profiles/config, run-status, actions, posts, interview batch, env-status). **Lake-only path note:** Neo4j entity listing/detail is under `GET /api/graph/entities/{graphId}/...` (not `/api/simulation/entities/...`) so `http.ServeMux` does not treat `simulation_id` values like `entities` as ambiguous with graph routes. If `LAKE_BACKEND_ROOT` / scripts are missing, runtime falls back to `simrunner.Disabled` (prepare still works; start returns an error).
6. **Report** — `usecase/report` drives outline + per-section generation (LLM + `ports.GraphTools`), writes Python-compatible artifacts via `ports.ReportRepository` (`adapters/reportstore`: `meta.json`, `outline.json`, `progress.json`, `section_NN.md`, `full_report.md`, `agent_log.jsonl`, `console_log.txt`). `adapters/neo4j/graphtools.go` implements keyword search + stats over `GraphStore.GetGraphData`. HTTP is Flask-equivalent for fishtank paths; **Lake-only:** `GET /api/report/check` and `GET /api/report/by-simulation` take `?simulation_id=` (path segments would conflict with `GET /api/report/{reportId}/agent-log` under Go 1.22+ `ServeMux`). Also: list, download, DELETE, progress, sections, single section, log streams, chat, `tools/search`, `tools/statistics`.
