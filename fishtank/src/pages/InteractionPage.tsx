...
export default function InteractionPage() {
  ...
  const [currentStatus, setCurrentStatus] = useState<'processing' | 'completed' | 'error' | 'ready'>('ready')
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        addLog('Failed to load interaction context')
      }
    })()
  }, [reportId, addLog])

  const statusClass = currentStatus === 'ready' ? 'completed' : currentStatus
  const statusText =
    currentStatus === 'error' ? 'Error' : currentStatus === 'processing' ? 'Working' : currentStatus === 'completed' ? 'Done' : 'Ready'
